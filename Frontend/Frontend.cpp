...
int Frontend::select_attrlist_from_table(char relname_source[ATTR_SIZE],
  ...
  return Algebra::project(relname_source, relname_target, attr_count,
                          attr_list);
  // return SUCCESS;
}
...
int Frontend::select_from_join_where(char relname_source_one[ATTR_SIZE],
  ...
  // Algebra::join
  return Algebra::join(relname_source_one, relname_source_two, relname_target,
                       join_attr_one, join_attr_two);
}
...
int Frontend::select_attrlist_from_join_where(
  ...
  int ret = Algebra::join(relname_source_one, relname_target, "temp",
                          join_attr_one, join_attr_two);
  if (ret != SUCCESS)
    return ret;
