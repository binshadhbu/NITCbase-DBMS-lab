...
int BlockBuffer::getHeader(HeadInfo *head)
{
    ...
    int ret = loadBlockAndGetBufferPtr(&bufferPtr); // Returns a pointer to the first byte of the buffer storing the block. This function will load the block to the buffer if it is not already present.
    if (ret != SUCCESS)
        return ret;
...
int BlockBuffer::loadBlockAndGetBufferPtr(unsigned char **buffPtr)
{
    ...
    if (bufferNum == E_BLOCKNOTINBUFFER)
    {
        ...
        Disk::readBlock(StaticBuffer::blocks[bufferNum], this->blockNum); // Read the block into the free buffer using readBlock()
    }
