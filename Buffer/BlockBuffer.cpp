...
//                 (recordSize * slotNum)); // calculate buffer + offset
...
// *NOTE: this function expects the caller to allocate memory for the argument (is
// this so?) ? in the function, it is simply pointing the buffer pointer to already
// alocated ? memory, thus it does not require the memory allocated
// */

// /*
//         * NOTE: This function will NOT check if the block has been initialised
//    as a
//         * record or an index block. It will copy whatever content is there in
//    that
//         * disk block to the buffer.

//         * Also ensure that all the methods accessing and updating the block's
//    data
//         * should call the loadBlockAndGetBufferPtr() function before the access
//    or
//         * update is done.
...
//   // the 		timestamps of all other occupied buffers in
//   // BufferMetaInfo.
...
//   } else if (bufferNum ==
//              E_BLOCKNOTINBUFFER) // the block is not present in the buffer
//   {
...
// //! Sets the indexNumth entry of the block with the input struct InternalEntry
// //! contents.
